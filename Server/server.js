...
let lastAdTimestamp = null;
let warned60 = false;
let warned30 = false;

function getDefaultPackSettings() {
    return {
        enabled: true
    };
}
...
function loadPackSettings() {
    try {
        if (!fs.existsSync(PACK_SETTINGS_FILE)) {
            fs.writeFileSync(
                PACK_SETTINGS_FILE,
                JSON.stringify(getDefaultPackSettings(), null, 2)
            );
        }

        return JSON.parse(fs.readFileSync(PACK_SETTINGS_FILE, "utf8"));
    } catch (err) {
        console.log("Could not load pack-settings.json:", err.message);
        return getDefaultPackSettings();
    }
}

function savePackSettings(settings) {
    fs.writeFileSync(
        PACK_SETTINGS_FILE,
...
function arePacksEnabled() {
    return loadPackSettings().enabled === true;
}
...
console.log("[TWITCH CHAT MSG]", tags["display-name"], message);
    ...
    if (command === "!packson") {
        if (!isBroadcaster) {
            twitchClient.say(channel, `@${username} only the broadcaster can use !packson.`);
            return;
        }
    
        savePackSettings({ enabled: true });
    
        twitchClient.say(channel, "Star Citizen card packs are now ENABLED.");
        return;
    }
    
    if (command === "!packsoff") {
        if (!isBroadcaster) {
            twitchClient.say(channel, `@${username} only the broadcaster can use !packsoff.`);
            return;
        }
    
        savePackSettings({ enabled: false });
    
        twitchClient.say(channel, "Star Citizen card packs are now DISABLED.");
        return;
    }
    
    if (command === "!packstatus") {
        twitchClient.say(
            channel,
            `Card packs are currently ${arePacksEnabled() ? "ENABLED" : "DISABLED"}.`
        );
    ...
    if (command === "!openpack") {
        ...
        if (!arePacksEnabled()) {
            twitchClient.say(channel, `@${username} card packs are only available during Star Citizen streams.`);
            return;
        }
...
async function checkStreamStatus() {
    try {
        ...
        if (!res.ok) {
            console.log("Stream status check failed:", data);
            return;
        }
        ...
        console.log("Stream status error:", err.message);
    }
}

async function checkAdSchedule() {
    try {
        const res = await fetch(
            `https://api.twitch.tv/helix/channels/ads?broadcaster_id=${BROADCASTER_ID}`,
            {
                headers: {
                    "Client-ID": CLIENT_ID,
                    "Authorization": `Bearer ${ACCESS_TOKEN}`
        ...
        if (!res.ok) {
            console.log("Ad schedule check failed:", data);
            return;
        }
        ...
        const secondsRemaining =
            Math.floor((nextAdTime - Date.now()) / 1000);

        if (secondsRemaining <= 60 && secondsRemaining > 30 && !warned60) {
            warned60 = true;

            announceChat(
                `📺 Ad break in approximately 60 seconds. Duration: ${duration} seconds.`
            );
        }

        if (secondsRemaining <= 30 && secondsRemaining > 0 && !warned30) {
            warned30 = true;

            announceChat(
                `📺 Ad break in approximately 30 seconds. Duration: ${duration} seconds.`
            );
        }

        if (secondsRemaining <= 0 && lastAdTimestamp !== nextAdTime) {
            lastAdTimestamp = nextAdTime;

            announceChat(
                `📺 Ad break is starting now. Back in around ${duration} seconds.`
            );

            setTimeout(() => {
                announceChat("✅ Ad break finished. Welcome back.");
            }, duration * 1000);
        ...
        console.log("Ad schedule error:", err.message);
    }
}
    ...
    ws.on("message", async (raw) => {
        ...
        if (messageType === "notification") {
            ...
            if (subType === "channel.follow") {
                ...
                announceChat(
                    `${EMOTES.wave} @${event.user_name} just followed! (${followerCount ?? history.totals.follows} followers) ${EMOTES.heart}`
                );
            }
            ...
            if (subType === "channel.subscription.gift") {
                ...
                if (arePacksEnabled()) {
                    announceChat(`${EMOTES.gift} @${gifter} gifted ${total} sub${total === 1 ? "" : "s"}! The gifter gets 1 pack to redeem with !openpack ${EMOTES.heart}`);
                } else {
                    announceChat(`${EMOTES.gift} @${gifter} gifted ${total} sub${total === 1 ? "" : "s"}! ${EMOTES.heart}`);
                }
            }
            ...
            if (event.is_prime) {
                ...
                if (arePacksEnabled()) {
                    announceChat(
                        `${EMOTES.gift} @${event.user_name} subscribed with Prime for ${months} month${months === 1 ? "" : "s"}! Redeem your pack with !openpack ${EMOTES.heart}`
                    );
                } else {
                    announceChat(
                        `${EMOTES.gift} @${event.user_name} subscribed with Prime for ${months} month${months === 1 ? "" : "s"}! ${EMOTES.heart}`
                    );
                }
                ...
                if (arePacksEnabled()) {
                    announceChat(`${EMOTES.gift} @${event.user_name} subscribed with ${event.tier} for ${months} months! Redeem your pack with !openpack ${EMOTES.heart}`);
                } else {
                    announceChat(`${EMOTES.gift} @${event.user_name} subscribed with ${event.tier} for ${months} months! ${EMOTES.heart}`);
                }
            ...
            if (subType === "channel.raid") {
                ...
                announceChat(`${EMOTES.raid} Welcome raiders! @${event.from_broadcaster_user_name} brought over ${event.viewers} viewers! ${EMOTES.salute}`);
                twitchClient.say(process.env.TWITCH_CHANNEL, `/shoutout ${event.from_broadcaster_user_login}`);
            }

            if (subType === "channel.cheer") {
                const bitsAmount = Number(event.bits) || 0;
            
                if (bitsAmount < 5) {
                    console.log(`Ignored cheer below 5 bits: ${bitsAmount}`);
                    return;
                }
                ...
                announceChat(`${EMOTES.cheer} @${event.user_name || "Anonymous"} cheered ${bitsAmount} bits! ${EMOTES.gg}`);
            }
...
checkAdSchedule();
setInterval(checkAdSchedule, 15000);
